...
// Bare 'fs'/'path' rather than the 'node:' prefix ON PURPOSE: Next's client
// webpack config resolves bare Node builtins to a `false` fallback, but the
// 'node:' scheme has no loader and fails the build outright even for a module
// that only ever runs server-side.
import fs from 'fs';
...
// Date formatting lives in lib/postDates.js, NOT here: those helpers are called
// from the rendered component, and importing them from this module would pin
// this module — and `fs` — into the client bundle.
