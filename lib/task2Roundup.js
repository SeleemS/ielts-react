...
// Discovery and pre-rendering must use the same content rule as the monthly
// route: months before the first usable prompt do not have a page.
export function listAvailableRoundupMonths(prompts, now = new Date(), count = 6) {
  return listRoundupMonths(now, count).filter((slug) =>
    buildMonthlyRoundup(prompts, slug, { now }).groups.length > 0
  );
}
