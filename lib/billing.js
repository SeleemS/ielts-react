...
// SKUs and PPP
// ---------------------------------------------------------------------------
// New purchases are limited to the two plans advertised on /pricing. 6-month,
// annual, and Exam Pass lookup keys remain below so existing subscriptions,
// historical webhook events, and account-only upgrades continue to map
// correctly.
export const CHECKOUT_SKUS = ['monthly', '3month'];
  ...
  switch (event.type) {
    ...
    case 'invoice.paid': {
      ...
      await recordBillingEvent(admin, {
        ...
        userId,
        sku: sub
          ? skuFromLookupKey(sub?.items?.data?.[0]?.price?.lookup_key)
          : null,
