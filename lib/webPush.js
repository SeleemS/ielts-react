...
import { validPushEndpoint } from './pushEndpoint';
...
export async function sendPush(subscription, payload, { webPush = null, ttl = 6 * 3600 } = {}) {
  // Revalidate persisted rows too: older rows did not have a provider policy.
  if (!validPushEndpoint(subscription?.endpoint)) {
    return { sent: false, gone: true, reason: 'invalid-push-endpoint' };
  }
  ...
  try {
    ...
    await client.sendNotification(
      ...
      { TTL: ttl, urgency: 'normal', timeout: 15000 }
    );
