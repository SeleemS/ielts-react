      ...
      return {
        ...
        html: shell({
          ...
          footer: payload.access_expires_at
            ? `Your Exam Pass ends on ${escapeHtml(new Date(payload.access_expires_at).toLocaleDateString('en-US', { timeZone: 'UTC' }))}. It never renews — there is nothing to cancel.`
    ...
    case 'checkout_abandoned': {
      ...
      return {
        ...
        html: shell({
          ...
          body:
            '<p>Every plan — including the one-time Exam Pass — comes with a <strong>14-day money-back guarantee</strong>: if Pro doesn’t help, ask within 14 days and we refund it — no forms, no questions. Prices are already set for your region.</p>',
