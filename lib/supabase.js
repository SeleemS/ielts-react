...
const SPEAKING_SELECT = `
  ...
  speaking_details ( part, part1_questions, cue_card, part3_followups,
                     model_answer_html, model_answer_meta )
`;
...
//     modelAnswerHtml, modelAnswerRationale, modelAnswerBand,
...
export async function getSpeakingItem(slugOrLegacyId) {
  ...
  const shaped = {
    ...
    part: d.part,
    topic: speakingTopicOf(d),
    // Original Band 8-9 model answer authored by our content pipeline
    // (speaking_details.model_answer_html + meta.{band, rationale}); the page
    // sanitizes before rendering and shows a free teaser / premium unlock.
    modelAnswerHtml: d.model_answer_html || '',
    modelAnswerRationale: d.model_answer_meta?.rationale || '',
    modelAnswerBand: d.model_answer_meta?.band || '8-9',
  };
...
// List published listening passages for a given IELTS Listening part (1-4).
// Mirrors listPassagesByQuestionType, but filters on listening_details.part
// (see supabase/migrations/20260802010000_listening_parts.sql) via an inner
// join so only classified passages come back. listening_details is 1:1 with
// passages, so no dedupe is needed.
// Returns the listPassages projection: [{ id: slug, legacyId, title, ... }].
export async function listPassagesByListeningPart(part) {
  const run = (includeBandStats) =>
    getSupabase()
      .from('passages')
      .select(
        [
          ...
          'difficulty',
          ...(includeBandStats ? ['average_user_band', 'band_submission_count'] : []),
          'listening_details!inner(part)',
        ].join(', ')
      )
      .eq('skill', SKILLS.listening)
      .eq('status', 'published')
      .eq('listening_details.part', part)
      .order('title', { ascending: true });
  ...
  return (response.data || []).map(toPassageListItem);
}
...
// Short-lived memo for the full per-skill lists getRelatedPractice needs.
// Every question page's getStaticProps calls it, so a full static build was
// re-fetching the same complete lists ~380 times (several minutes of build
// time). Keyed per skill with a small TTL so ISR lambdas never serve stale
// lists for long.
const relatedListCache = new Map();
const RELATED_LIST_TTL_MS = 5 * 60 * 1000;
async function cachedList(key, loader) {
  const hit = relatedListCache.get(key);
  if (hit && Date.now() - hit.at < RELATED_LIST_TTL_MS) return hit.value;
  const value = await loader();
  relatedListCache.set(key, { at: Date.now(), value });
  return value;
}
...
export async function getRelatedPractice(skill, currentSlug, preferred = null, limit = 3) {
  ...
  if (skill === SKILLS.speaking) {
    preferredItems = (await cachedList('speaking', listSpeakingItems)).filter(
      (item) => preferred == null || item.part === preferred
    );
  } else if (preferred) {
    preferredItems = (
      await cachedList(`types:${skill}`, () => listPassages(skill, { withQuestionTypes: true }))
    ).filter((item) => (item.questionTypes || []).includes(preferred));
  }

  const fallbackItems =
    skill === SKILLS.speaking
      ? await cachedList('speaking', listSpeakingItems)
      : await cachedList(`list:${skill}`, () => listPassages(skill));
  const seen = new Set();
...
export async function getMockTest(slug) {
  ...
  const rawSections = (data.mock_test_sections || [])
    .slice()
    .sort((a, b) => a.position - b.position);
  // Sections are independent — fetch them concurrently instead of serially
  // (a 4-section mock was 6-10 sequential round trips).
